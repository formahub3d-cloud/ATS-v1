...
export default function GlassCalendarPicker({
  ...
  const statusColor = (day: CalendarDay) => {
    if (!day.isCurrentMonth) return 'bg-transparent text-text-muted/30 border-transparent'
    switch (day.status) {
      case 'available':
        return 'bg-success/15 text-success border-success/40'
      case 'unavailable':
        return 'bg-[rgba(255,255,255,0.02)] text-text-secondary border-[rgba(255,255,255,0.05)]'
      case 'holiday':
    ...
    <div className="w-full">
      ...
      <div className="flex items-center justify-between mb-4">
        <motion.button
          ...
          className="w-9 h-9 rounded-xl flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-[rgba(255,255,255,0.04)] transition-colors border border-[rgba(255,255,255,0.06)] backdrop-blur-md"
        >
      ...
      <div className="grid grid-cols-7 gap-1">
        {calendarDays.map((day, index) => (
          <motion.button
            ...
            whileHover={day.isCurrentMonth ? { scale: 1.08, zIndex: 10 } : {}}
            onClick={() => toggleDay(day)}
            className={cn(
              'relative aspect-square rounded-lg flex flex-col items-center justify-center text-sm font-medium border transition-all duration-250 backdrop-blur-sm',
              statusColor(day),
      ...
      <div className="flex items-center gap-4 mt-4 text-xs">
        ...
        <div className="flex items-center gap-1.5">
          <div className="w-3 h-3 rounded bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.05)]" />
      ...
      <div className="flex gap-2 mt-4">
        <motion.button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-sky-primary bg-[rgba(91,184,245,0.08)] border border-[rgba(91,184,245,0.12)] rounded-lg hover:bg-[rgba(91,184,245,0.12)] transition-colors backdrop-blur-md"
        >
        ...
        <motion.button
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-text-muted bg-[rgba(255,255,255,0.04)] border border-[rgba(255,255,255,0.06)] rounded-lg hover:bg-[rgba(255,255,255,0.08)] transition-colors backdrop-blur-md"
        >
